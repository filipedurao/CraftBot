...
if(!grupo) return message.reply("<a:errado:687759347027017744> Erro no Comando!\n Nenhum grupo foi encontrada.!\n Use: /setargrupoticket @grupo_aqui");
if (grupo) {
  try{
    await connection.query(
    `UPDATE guildconfig SET grupoticket = '${grupo.id}' WHERE guildId = '${message.guild.id}'`
    )
    let embed = new Discord.MessageEmbed()

    .setColor("#964b00") 
    .setTitle(`${message.author.username}`) 
    .setDescription(`**Grupo de suporte ticket foi setado com sucesso! <a:verify:687759351330766901> \n \n Setado como: **\n> ${grupo} `)
