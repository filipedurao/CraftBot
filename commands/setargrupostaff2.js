...
if(!grupo) return message.reply("<a:errado:687759347027017744> Erro no Comando!\n Nenhum grupo foi encontrada.!\n Use: /setargrupostaff2 @grupo_aqui");
if (grupo) {
  try{
    await connection.query(
    `UPDATE guildconfig SET staff2 = '${grupo.id}' WHERE guildId = '${message.guild.id}'`
    )
    let embed = new Discord.MessageEmbed()

    .setColor("#964b00") 
    .setTitle(`${message.author.username}`) 
    .setDescription(`**Grupo do 2 menor cargo da staff setado com sucesso! <a:verify:687759351330766901> \n \n Setada como: **\n> ${grupo} `)
