...
// COMANDO ---------------------
        ...
        if(!novoIP) return message.reply("<a:errado:687759347027017744> Erro no Comando!\n Nenhum id foi encontrada.!\n Use: /setaridcategoriaticket <id>");
        if (novoIP) {
          try{
            await connection.query(
            `UPDATE guildconfig SET idcategoriaticket = '${novoIP}' WHERE guildId = '${message.guild.id}'`
            )
            let embed = new Discord.MessageEmbed()

            .setColor("#964b00") 
            .setTitle(`${message.author.username}`) 
            .setDescription(`**ID da categoria setada com sucesso! <a:verify:687759351330766901> \n \n  O id foi setado como: **\n${novoIP}`)
